...
int** MultiThreadHashChecker::ptabOverlap;
...
{
	MultiThreadHashChecker::ptabOverlap=new int*[contigSize];
	for(int i=0;i<contigSize;i++)
	{
		MultiThreadHashChecker::ptabOverlap[i]=new int[contigSize];
	}
	
	for(int i=0;i<contigSize;i++)
	{
		for(int j=0;j<contigSize;j++)
			MultiThreadHashChecker::ptabOverlap[i][j]=0;
	}
	...
	for(map<uint32_t, VNode>::iterator it=mapKmersV1.begin(); it!=mapKmersV1.end(); it++)
	{
		...
		for(int i=0;i<nodeSize;i++)
		{
			...
			for(int j=i+1;j<nodeSize;j++)
			{
				...
				if( py>px )
					MultiThreadHashChecker::ptabOverlap[px][py]++;
				else if(px>py)
					MultiThreadHashChecker::ptabOverlap[py][px]++;
			}
		}
	}
...
{
	//create a table to record which two are potentially overlap
	for(int i=0;i<contigSize;i++)
	{
		for(int j=0;j<contigSize;j++)
			if(ptabOverlap[i][j]>=minSupport)
				cout<<i<<" "<<j<<" "<<MultiThreadHashChecker::ptabOverlap[i][j]<<endl;
	}
...
void MultiThreadHashChecker::releaseHashCheckTab()
{
	...
	for(int i=0;i<contigSize;i++)
	{
		if(MultiThreadHashChecker::ptabOverlap[i]!=NULL)
		{
			delete[] MultiThreadHashChecker::ptabOverlap[i];
			MultiThreadHashChecker::ptabOverlap[i]=NULL;
		}
	}
	if(MultiThreadHashChecker::ptabOverlap!=NULL)
	{
		delete[] MultiThreadHashChecker::ptabOverlap;
		MultiThreadHashChecker::ptabOverlap=NULL;
	}
}
